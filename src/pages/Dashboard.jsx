  ...
  return (
    <div className="w-full flex flex-col gap-6 -mt-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-theme-bg-card rounded-2xl shadow-sm p-6 flex flex-col justify-between border border-theme-border">
          <div className="text-theme-text-muted text-sm font-semibold mb-3 flex items-center justify-between">
            Total Balance
            <span className="flex items-center gap-1 bg-theme-primary-btn/10 text-theme-primary-btn px-2 py-0.5 rounded text-xs font-bold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
         <div className="bg-white border border-theme-border shadow-sm rounded-2xl p-6 relative z-0 overflow-hidden">
             <div className="text-theme-text-main text-base font-bold w-full text-left font-heading mb-4">Spending Trends</div>
             <div className="w-full flex items-center justify-center">
                 <MonthlyBarChart />
             </div>
         </div>
         <div className="bg-white border border-theme-border shadow-sm rounded-2xl p-6 relative z-0 overflow-hidden">
             <div className="text-theme-text-main text-base font-bold w-full text-left font-heading mb-4">Expense Categories</div>
             <div className="w-full flex items-center justify-center">
                 <CategoryPieChart />
             </div>
         </div>
      ...
      <div className="bg-theme-bg-card border border-theme-border rounded-2xl overflow-hidden shadow-sm mt-2">
        ...
        <div className="w-full overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-theme-border/50 text-theme-text-muted font-semibold border-b border-theme-border">
              <tr>
            ...
            <tbody className="divide-y divide-theme-border">
              {recentTx.map(t => (
                <tr key={t.id} className="hover:bg-theme-border/20 transition-colors">
                  ...
                  <td className={`px-6 py-4 font-mono font-medium ${t.type === 'income' ? 'text-theme-primary-btn' : 'text-theme-text-main'}`}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2 text-theme-text-main font-medium">
                      <span className={`w-2.5 h-2.5 rounded-full ${isThisMonth(t.date) ? 'bg-theme-primary-btn' : 'bg-theme-text-muted'}`}></span>
